...
def max_scenic_score(inp):
    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            n_trees_left = count_trees_shorter(np.flip(current_row_higher[:j]))
            n_trees_right = count_trees_shorter(current_row_higher[j+1:])
            n_trees_top = count_trees_shorter(np.flip(current_column_higher[:i]))
            n_trees_bottom = count_trees_shorter(current_column_higher[i+1:])
...
if __name__ == "__main__":
    #--- Test
    test_content = "30373\n25512\n65332\n33549\n35390".split("\n")
    test_content = [list(i) for i in test_content]

    n_visible = count_non_visible_trees(test_content)
    assert n_visible == 21, "Expected task1 test output to be 21"

    max_scenic = max_scenic_score(test_content)
    assert max_scenic == 8, "Expected task2 test output to be 8"
